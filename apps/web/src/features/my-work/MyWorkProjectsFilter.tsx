/**
 * The My Work "Projects" toolbar filter: a neutral toolbar toggle
 * that opens a multi-select popover of the Projects represented by
 * the loaded canonical My Work payload (narrowed to the selected
 * Research Groups while a group filter is active).
 *
 * It reuses the established Research Group filter's visual and
 * interaction pattern exactly (the same toggle, popover, search,
 * checkbox, and Escape/outside-click contract).
 *
 * This component is a controlled PRESENTATION component: it renders
 * whatever `selectedIds` (the persisted `preferences.projectIds`)
 * say and reports user intent through `onToggle` / `onClear`. It
 * owns only transient UI concerns — the open/closed popover state is
...
interface MyWorkProjectsFilterProps {
  /**
   * The Projects represented by the loaded My Work context, already
   * narrowed to the active Research Group scope (the canonical
   * option set — derived from the payload, never fetched).
   */
  options: readonly ProjectFilterOption[]
  /** The persisted selection (`preferences.projectIds`). */
  selectedIds: readonly number[]
  /** Controlled popover open state. */
  open: boolean
  onOpenChange: (open: boolean) => void
  /** Toggle one Project's membership in the selection. */
  onToggle: (projectId: number, checked: boolean) => void
  /** Clear the whole Project selection (only `projectIds`). */
  onClear: () => void
}

// The search field appears only above this number of available
// projects (the approved "more than 8" threshold, same as the
// Research Group filter).
const SEARCH_THRESHOLD = 8

export const MyWorkProjectsFilter =
  forwardRef<
    HTMLButtonElement,
    MyWorkProjectsFilterProps
  >(function MyWorkProjectsFilter(
    {
      options,
      selectedIds,
      open,
