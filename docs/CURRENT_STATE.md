...
- **IMPLEMENTED (DOMAIN; API/UI NOT WIRED)** — `MeetingItemFollowUp` stores a concrete scheduled continuation separately from `MeetingItem.outcome`, with required target Meeting, target MeetingSection, target MeetingItem, lifecycle status, creator, and timestamps. Database constraints enforce a distinct source/target item and at most one non-cancelled follow-up per source item. `schedule_meeting_item_follow_up` requires an existing upcoming target Meeting and an explicit visible Section belonging to it, reuses the canonical Meeting write permission and agenda append rules, atomically creates a new `not_discussed` target item plus the trace, copies only the source title, and changes the source outcome only after persistence succeeds. Identical retries reuse the active schedule; a different target is rejected pending a later Reschedule operation. No API/client wiring, reschedule/cancel behavior, automatic deletion/cancellation transitions, Meeting creation, Section suggestion, or Topic persistence is implemented; the existing direct `Follow up` outcome action remains unchanged.
