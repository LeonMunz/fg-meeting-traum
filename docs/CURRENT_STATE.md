...
- **NOT IMPLEMENTED (deferred)** — e-mail delivery, the frontend live password-policy validation (wiring of `registration-password-policy` into the Create Account UI, incl. debouncing), e-mail verification, password reset/recovery, membership invitations, invitation rate limiting, invitation audit history, global normalized User-email uniqueness, and account merging.
