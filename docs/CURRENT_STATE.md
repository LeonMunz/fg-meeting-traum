...
- **IMPLEMENTED (PERSISTENCE ONLY)** — `MeetingItemFollowUp` stores a concrete scheduled continuation separately from `MeetingItem.outcome`, with required target Meeting, target MeetingSection, target MeetingItem, lifecycle status, creator, and timestamps. Database constraints enforce a distinct source/target item and at most one non-cancelled follow-up per source item. The later scheduling operation will accept only an existing upcoming Meeting, create the target item and follow-up atomically, and change the source outcome only after success. Scheduling operations, APIs, UI, reschedule/cancel behavior, and automatic deletion/cancellation transitions are not implemented.
